...
MUTATIONS = [
    ...
    ("PE24", "app/lib/evidence.js",
     ...
     ["the page's own words claim nothing the documents do not"]),
    # A pool's stage 4, "Passed, waiting for a key" (app/lib/stages.js). "PS": the second window's prefix,
    # run by node on its own test file. The stage was appended, so every reading of a stage by its order
    # ("2 or later", "not 0") has to be checked against the one that now comes after Closing.
    ("PS1", "app/lib/stages.js",
     'export const STAGE_IDS = ["Idle", "Challenge", "Funded", "Closing", "PassedAwaitingKey"];',
     'export const STAGE_IDS = ["Idle", "Challenge", "Closing", "Funded", "PassedAwaitingKey"];',
     ["the app's stages are the contract's, in its order, and anything more comes after them"]),
    ("PS2", "app/lib/stages.js",
    ...
    ("PS3", "app/lib/stages.js",
     "  return STAGE[Number(stage)] ?? `Stage ${Number(stage)}`;",
     "  return STAGE[Number(stage)];",
     ["stage 4 is the trader who passed and waits for a key, and no stage is shown as undefined"]),
    ("PS4", "app/lib/stages.js",
     "  return Number(stage) === 2 || Number(stage) === 3;",
     "  return Number(stage) >= 2;",
     ["a funded trader is shown for a running or settling stage, not for one still waiting for its key"]),
    ("PS5", "app/lib/funding.js",
     '  if (Number(stage) !== 0) return { kind: "taken" };',
     '  if (Number(stage) >= 1 && Number(stage) <= 3) return { kind: "taken" };',
     ["a pool waiting for a key is taken: it sells no challenge"]),
    ("PS6", "app/lib/verdict.js",
     '  return kind === "pool" && Number(cutBlock) > 0 && Number(stage) === 0;',
     '  return kind === "pool" && Number(cutBlock) > 0 && Number(stage) !== 2 && Number(stage) !== 3;',
     ["a pool waiting for a key is not read as a funded stage that is over"]),
    ("PS7", "app/lib/stages.js",
...
RUNNERS = {
    ...
    # A pool's stage 4 in the app: its own test file.
    "PS": (["node", "--test", "--test-reporter=tap", "app/tests/stages.test.mjs"], r"^\s*not ok \d+ - (.+?)\s*$"),

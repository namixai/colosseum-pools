...
M = [
 ...
 ("задержка не применяется", "    k = min(j + lag, n - 1)", "    k = j"),
 ("выход по закрытию минуты, а не по открытию", "    p_out = p_line if lag == 0 else float(o[k])",
  "    p_out = p_line if lag == 0 else float(bar['c'][k])"),
 ("гэп через линию исполнен по линии", "        p_out = float(o[j])                                 # минута открылась за линией: раньше не исполнить",
  "        pass"),
 ("путь до ликвидации включает минуту выхода целиком",
  "        deep = min(float(l[i:k].min()), tail) if side > 0 else max(float(h[i:k].max()), tail)",
  "        deep = min(float(l[i:k + 1].min()), tail) if side > 0 else max(float(h[i:k + 1].max()), tail)"),
 ("ликвидация биржей не обрезает убыток", "        p_out = p_liq\n    eq_out", "        pass\n    eq_out"),

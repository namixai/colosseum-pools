...
| Contracts `PoolFactory`, `Pool`, `ChallengeAccount`, `KeyRegistry` (Foundry, HyperEVM testnet) | 16–17 Sep: written, a mutation check of the tests, two internal review passes and their fixes. 17 Sep, after a bot review: the funded trader's share is computed from what closing realized, settlement waits for positions nobody named, and the factory takes an operator-set fee per challenge. These last changes have had only the author's review so far. 67 simulator tests; not deployed. Later on 17 Sep, from the live spike: no HyperEVM deposit (capital arrives on HyperCore), the 1 USDC fee for a new HyperCore account is reserved or taken from the share, and a stop records the key it cut. A second internal review pass the same day found that a published key could be spoiled by sending it USDC, and fixed it. 75 simulator tests; not deployed. 20 Sep: the trader's share became two numbers, one for the profit made on the challenge and one for the profit made on the funded account, so a pool can pay for performance on its own capital without paying the same rate for passing the audition; either may be zero. A price of zero is now refused at creation: a pool that gives challenges away spends the investor's capital and a published agent key on nothing. 77 simulator tests, 208 mutations. The rehearsal deployment of 18 Sep predates this change, so its terms no longer match the app. Later on 20 Sep: `Rules` and `Terms` are repeated in three languages -- Solidity defines them, the app and the agents each carry a copy -- and a copy left behind is a silent failure, because the app then decodes a pool's terms into the wrong fields and the first anyone notices is a pool that will not open. CI now reads the compiled ABI and compares both copies, types and names, and a second script falsifies that check by drifting each copy in turn, including a swap of two fields of the same width |
| Pool gateway, which checks on chain that a key belongs to the trader's account before an order goes to the signer | 17 Sep: written. Later that day it got its own demo signer, which the demo uses: testnet keys in owner-only files, and the platform's caps checked before signing. The mode that calls a Usenami Signer gateway stays in the code, unused. The trader's wallet signs the order's own fields (EIP-712). Offline tests, two of them against the Hyperliquid SDK's published signing vectors. Not running anywhere yet. 18 Sep: a sell counts toward the 400 USDC cap at its limit or at the market mid, whichever is higher. Counted at its limit alone, a sell priced under the market would pass the cap and fill over it; the AI trader's own per-order cap had the same gap. Later on 18 Sep, the CTO's decision: the notional cap is on opening and growing a position, so a reduce-only order passes it; its size cap and every other check still bind it. |
| App: investor page, list of pools, challenge page, and the check-it-yourself page | 17 Sep: written as static files and tried in a browser against testnet reads and the live Signer attestation. No contracts are deployed yet, so there is no pool to show. Later that day the Signer attestation panel was removed, since the demo doesn't use the enclave. Later that day: the investor's capital goes to a pool as a HyperCore transfer signed by the wallet, and the request the app sends was accepted on testnet. 20 Sep: section 1 of the check-it-yourself page was answering out of the event log, which meant 25 HTTP requests carrying 58 JSON-RPC calls on a cold load; the public RPC refused them and the section showed a raw error to every visitor. The scan could not have found what it looked for either — 40 windows of 50 blocks reach 2 000 of the 199 832 blocks since the deployment, and that gap grows by some 88 000 blocks a day. The section now answers from contract state, which is exact: the approved agent key, the registry's binding, and the block and key of the last stop, which the account writes down itself. Measured after the change: 3 HTTP requests for the whole page. The windowed scan is gone from the code, not just from the page. 20 Sep: an Economics page, which runs the pool calculator in the browser -- the closed formulas and the grid of a Monte Carlo model of these contracts, held to that model's own numbers by golden cases in CI. It is its own page on purpose: it describes a pool of several seats with a shared reserve, while the new-pool form makes a single-seat pool, and standing them together would say the numbers are about the pool being created. One figure does travel to that form, the floor under the challenge price of that pool. Next to the model it shows a measurement, kept apart from it: what a cascade like 10 Oct 2025 does to these rules, replayed on one-minute bars, always printed together with the list of coins it was measured on. The list decides the size of the tail -- on that day, at the same leverage and under the same rules, a pool holding BTC, ETH and SOL lost between a quarter and nine tenths of its seats' capital depending on which coin the big seats sat on, and a pool on a wide list of alts lost more than four fifths. A single headline figure would be wrong for one of them, so the page prints neither without the list. The measurement carries no frequency and is never subtracted from the returns. The same day, found while wiring that figure in: the new-pool page did not load at all. It asked isPlatformAsset for 64 perps in one Promise.all, ethers sends that as one JSON-RPC batch, and the public RPC refuses a batch over twenty calls (-32010). Candidates now come from the deployment record and the chain still decides; fan-out reads go through a helper bounded to twenty |
...
| Hosting: the gateway and the keeper on their own host, the app on a Cloudflare Worker | 18 Sep: the host set up from `ops/host/`, the agent keys and the keeper's key made on it, and the rehearsal deployment published on testnet. The gateway and the keeper run there; the gateway is reached through an ssh tunnel until the public name is set up. Later that day the gateway went public as `pools-api.usenami.io` behind Cloudflare, and the app went up at `pools.usenami.io`: commit `7855198`, on a Worker with static assets rather than Pages. The checkpoint set for 26 Sep passed that day: a pool with its capital, a challenge bought, its key bound, an order through the gateway filled, and an order over the cap refused before signing. The keeper activated that challenge on its own |
| Access for US residents | 17 Sep: planned closed, with a geoblock at the host, a confirmation on entry and a line in the terms. 18 Sep, Alex's word: a testnet demo with mock funds stays open to everyone. The US wording is gone from the app, and the entry question only asks the visitor to confirm this is a testnet demo. No geoblock |
